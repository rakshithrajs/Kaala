...
from kaala.storage.models import ScheduledPrompt
...
class ScheduleRepository:
    ...
    async def mark_executed(
        ...
        if prompt:
            ...
            prompt.executed_at = executed_at or datetime.now(timezone.utc)
            await self.session.commit()
            await self.session.refresh(prompt)
        return prompt
    ...
    async def mark_failed(self, prompt_id: int) -> Optional[ScheduledPrompt]:
        ...
        if prompt:
            ...
            prompt.executed_at = datetime.now(timezone.utc)
            await self.session.commit()
            await self.session.refresh(prompt)
        return prompt
